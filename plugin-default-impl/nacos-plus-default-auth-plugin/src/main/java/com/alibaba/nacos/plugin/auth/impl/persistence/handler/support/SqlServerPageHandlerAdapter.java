 ...
 * @author huangKeMing
 */
public class SqlServerPageHandlerAdapter implements PageHandlerAdapter {
    
    @Override
    public boolean supports(String dataSourceType) {
        return PersistenceConstant.SQLSERVER.equals(dataSourceType);
    }
    ...
    public OffsetFetchResult addOffsetAndFetchNext(String fetchSql, Object[] arg, int pageNo, int pageSize) {
        if (!fetchSql.contains(AuthPageConstant.OFFSET)) {
            if(!fetchSql.contains(AuthPageConstant.ORDER_BY)){
                fetchSql += " " + AuthPageConstant.ORDER_BY_ID;
            }
