...
public class ConfigInfoMapperBySqlServerTest {
    ...
    public void testFindConfigInfoByAppFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id,data_id,group_id,tenant_id,app_name,content FROM config_info WHERE tenant_id LIKE ? AND app_name= ?  ORDER BY id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), new Object[] {tenantId, appName});
    }
    ...
    public void testGetTenantIdList() {
        ...
        Assert.assertEquals(mapperResult.getSql(), "SELECT tenant_id FROM config_info WHERE tenant_id != '"+ NamespaceUtil.getNamespaceDefaultId() +"' GROUP BY tenant_id ORDER BY tenant_id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), emptyObjs);
    }
    ...
    public void testGetGroupIdList() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT group_id FROM config_info WHERE tenant_id ='' GROUP BY group_id ORDER BY group_id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), emptyObjs);
    }
    ...
    public void testFindAllConfigKey() {
        ...
        Assert.assertEquals(mapperResult.getSql(), " SELECT data_id,group_id,app_name  FROM (  SELECT id FROM config_info WHERE tenant_id LIKE ? ORDER BY id OFFSET "+context.getStartRow()+" ROWS FETCH NEXT "+context.getPageSize()+" ROWS ONLY ) g, config_info t WHERE g.id = t.id  ");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), new Object[] {tenantId});
    }
    ...
    public void testFindAllConfigInfoBaseFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT t.id,data_id,group_id,content,md5 FROM ( SELECT id FROM config_info ORDER BY id OFFSET "+context.getStartRow()+" ROWS FETCH NEXT "+context.getPageSize()+" ROWS ONLY ) g, config_info t  WHERE g.id = t.id ");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), emptyObjs);
    }
    ...
    public void testFindAllConfigInfoFragment() {
        ...
        Assert.assertEquals("SELECT id,data_id,group_id,tenant_id,app_name,md5,gmt_modified,type,encrypted_data_key "
                        + "FROM config_info WHERE id > ? ORDER BY id ASC" + " OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY",
    ...
    public void testFindChangeConfig() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id, data_id, group_id, tenant_id, app_name,md5, gmt_modified, encrypted_data_key FROM config_info WHERE gmt_modified >= ? and id > ?  ORDER BY id  OFFSET 0 ROWS FETCH NEXT ? ROWS ONLY");
    ...
    public void testFindChangeConfigFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id,data_id,group_id,tenant_id,app_name,type,md5,gmt_modified FROM config_info WHERE  1=1  AND tenant_id = ?  AND app_name = ?  AND gmt_modified >=?  AND gmt_modified <=?  AND id > 100 ORDER BY id ASC OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
    ...
    public void testListGroupKeyMd5ByPageFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT t.id,data_id,group_id,tenant_id,app_name,md5,type,gmt_modified,encrypted_data_key FROM ( SELECT id FROM config_info ORDER BY id OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY ) g, config_info t WHERE g.id = t.id");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), emptyObjs);
    ...
    public void testFindConfigInfoBaseLikeCountRows() {
        MapperResult mapperResult = configInfoMapperBySqlServer.findConfigInfoBaseLikeCountRows(context);
    ...
    public void testFindConfigInfoBaseLikeFetchRows() {
        MapperResult mapperResult = configInfoMapperBySqlServer.findConfigInfoBaseLikeFetchRows(context);
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id,data_id,group_id,tenant_id,content FROM config_info WHERE  1=1 AND tenant_id=''  ORDER BY id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), emptyObjs);
    }
    ...
    public void testFindConfigInfo4PageFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id,data_id,group_id,tenant_id,app_name,content,type,encrypted_data_key FROM config_info WHERE  tenant_id=?  AND app_name=?   ORDER BY id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), new Object[] {tenantId, appName});
    }
    ...
    public void testFindConfigInfoBaseByGroupFetchRows() {
        ...
        MapperResult mapperResult = configInfoMapperBySqlServer.findConfigInfoBaseByGroupFetchRows(context);
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id,data_id,group_id,content FROM config_info WHERE group_id=? AND tenant_id=?  ORDER BY id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
    ...
    public void testFindConfigInfoLike4PageCountRows() {
        MapperResult mapperResult = configInfoMapperBySqlServer.findConfigInfoLike4PageCountRows(context);
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT count(*) FROM config_info WHERE  tenant_id LIKE ?  AND app_name = ? ");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), new Object[] {tenantId, appName});
    }
    ...
    public void testFindConfigInfoLike4PageFetchRows() {
        MapperResult mapperResult = configInfoMapperBySqlServer.findConfigInfoLike4PageFetchRows(context);
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT id,data_id,group_id,tenant_id,app_name,content,encrypted_data_key FROM config_info WHERE  tenant_id LIKE ?  AND app_name = ?   ORDER BY id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY");
        Assert.assertArrayEquals(mapperResult.getParamList().toArray(), new Object[] {tenantId, appName});
    }
    ...
    public void testFindAllConfigInfoFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5  FROM (  SELECT id FROM config_info WHERE tenant_id LIKE ? ORDER BY id OFFSET ? ROWS FETCH NEXT ? ROWS ONLY ) g, config_info t  WHERE g.id = t.id ");
    ...
    public void testGetDataSource() {
        ...
        Assert.assertEquals(sql, DataSourceConstant.SQLSERVER);
    }
    ...
    public void testUpdateConfigInfoAtomicCas() {
        ...
        MapperResult mapperResult = configInfoMapperBySqlServer.updateConfigInfoAtomicCas(context);
