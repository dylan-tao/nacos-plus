...
public class HistoryConfigInfoMapperByPostgresqlTest {
    ...
    public void testRemoveConfigHistory() {
        MapperResult mapperResult = historyConfigInfoMapperByPostgresql.removeConfigHistory(context);
        Assert.assertEquals(mapperResult.getSql(), "DELETE FROM his_config_info WHERE gmt_modified < ? OFFSET 0 LIMIT ?");
    ...
    public void testFindConfigHistoryCountByTime() {
        MapperResult mapperResult = historyConfigInfoMapperByPostgresql.findConfigHistoryCountByTime(context);
    ...
    public void testFindDeletedConfig() {
        MapperResult mapperResult = historyConfigInfoMapperByPostgresql.findDeletedConfig(context);
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT data_id, group_id, tenant_id,gmt_modified,nid FROM his_config_info WHERE op_type = 'D' AND gmt_modified >= ? and nid > ? ORDER BY nid OFFSET 0 LIMIT ? ");
    ...
    public void testFindConfigHistoryFetchRows() {
        ...
        context.putWhereParameter(FieldConstant.DATA_ID, dataId);
        MapperResult mapperResult = historyConfigInfoMapperByPostgresql.findConfigHistoryFetchRows(context);
        Assert.assertEquals(mapperResult.getSql(),
    ...
    public void testDetailPreviousConfigHistory() {
        ...
        MapperResult mapperResult = historyConfigInfoMapperByPostgresql.detailPreviousConfigHistory(context);
        Assert.assertEquals(mapperResult.getSql(),
