...
public class ConfigInfoTagMapperByPostgresqlTest {
    ...
    public void testUpdateConfigInfo4TagCas() {
        ...
        MapperResult mapperResult = configInfoTagMapperByPostgresql.updateConfigInfo4TagCas(context);
    ...
    public void testFindAllConfigInfoTagForDumpAllFetchRows() {
        MapperResult mapperResult = configInfoTagMapperByPostgresql.findAllConfigInfoTagForDumpAllFetchRows(context);
        Assert.assertEquals(mapperResult.getSql(),
                " SELECT t.id,data_id,group_id,tenant_id,tag_id,app_name,content,md5,gmt_modified  FROM (  SELECT id FROM config_info_tag  ORDER BY id OFFSET 0 LIMIT 5 ) g, config_info_tag t  WHERE g.id = t.id  ");
