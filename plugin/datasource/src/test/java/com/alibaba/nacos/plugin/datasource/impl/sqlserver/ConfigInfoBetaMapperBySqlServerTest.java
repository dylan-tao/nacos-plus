...
public class ConfigInfoBetaMapperBySqlServerTest {
    ...
    public void testUpdateConfigInfo4BetaCas() {
        ...
        MapperResult mapperResult = configInfoBetaMapperBySqlServer.updateConfigInfo4BetaCas(context);
    ...
    public void testFindAllConfigInfoBetaForDumpAllFetchRows() {
        MapperResult result = configInfoBetaMapperBySqlServer.findAllConfigInfoBetaForDumpAllFetchRows(context);
        String sql = result.getSql();
        List<Object> paramList = result.getParamList();
        Assert.assertEquals(sql,
                " SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,beta_ips,encrypted_data_key  FROM ( SELECT id FROM config_info_beta  ORDER BY id OFFSET "+startRow+" ROWS FETCH NEXT "+pageSize+" ROWS ONLY )  g, config_info_beta t WHERE g.id = t.id ");
