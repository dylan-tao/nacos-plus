...
public class ConfigInfoAggrMapperByOracleTest {
    ...
    public void testBatchRemoveAggr() {
        ...
        MapperResult mapperResult = configInfoAggrMapperByOracle.batchRemoveAggr(context);
    ...
    public void testAggrConfigInfoCount() {
        ...
        context.putWhereParameter(FieldConstant.GROUP_ID, groupId);
        context.putWhereParameter(FieldConstant.TENANT_ID, tenantId);
        MapperResult mapperResult = configInfoAggrMapperByOracle.aggrConfigInfoCount(context);
        String sql = mapperResult.getSql();
        List<Object> paramList = mapperResult.getParamList();
    ...
    public void testFindConfigInfoAggrIsOrdered() {
        ...
        MapperResult mapperResult = configInfoAggrMapperByOracle.findConfigInfoAggrIsOrdered(context);
        String sql = mapperResult.getSql();
        List<Object> paramList = mapperResult.getParamList();
    ...
    public void testFindConfigInfoAggrByPageFetchRows() {
        ...
        Assert.assertEquals(sql,
                "SELECT * FROM (SELECT data_id,group_id,tenant_id,datum_id,app_name,content, ROWNUM as rnum FROM config_info_aggr WHERE data_id= ? AND group_id= ? AND tenant_id= ? ORDER BY datum_id) WHERE  rnum >= 1 and 5 >= rnum");
        Assert.assertEquals(paramList, Arrays.asList(dataId, groupId, tenantId));
    }
    
    @Test
    public void testFindAllAggrGroupByDistinct() {
        MapperResult mapperResult = configInfoAggrMapperByOracle.findAllAggrGroupByDistinct(null);
