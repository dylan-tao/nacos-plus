...
public class ConfigInfoBetaMapperByOracleTest {
    
    private ConfigInfoBetaMapperByOracle configInfoBetaMapperByOracle;
    ...
    public void setUp() throws Exception {
        configInfoBetaMapperByOracle = new ConfigInfoBetaMapperByOracle();
    ...
    public void testUpdateConfigInfo4BetaCas() {
        ...
        MapperResult mapperResult = configInfoBetaMapperByOracle.updateConfigInfo4BetaCas(context);
    ...
    public void testFindAllConfigInfoBetaForDumpAllFetchRows() {
        MapperResult result = configInfoBetaMapperByOracle.findAllConfigInfoBetaForDumpAllFetchRows(context);
        String sql = result.getSql();
        List<Object> paramList = result.getParamList();
        Assert.assertEquals(sql,
                " SELECT * FROM (SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,beta_ips,encrypted_data_key  FROM ( SELECT * FROM (SELECT id, ROWNUM as rnum FROM config_info_beta  ORDER BY id) WHERE  rnum >= 1 and 5 >= rnum )  g, config_info_beta t WHERE g.id = t.id ");
        Assert.assertEquals(paramList, Arrays.asList(startRow, pageSize));
    }
    
    @Test
    public void testGetTableName() {
        String tableName = configInfoBetaMapperByOracle.getTableName();
        Assert.assertEquals(tableName, TableConstant.CONFIG_INFO_BETA);
    }
    
    @Test
    public void testGetDataSource() {
        String dataSource = configInfoBetaMapperByOracle.getDataSource();
        Assert.assertEquals(dataSource, DataSourceConstant.ORACLE);
    }
}
