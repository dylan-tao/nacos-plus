...
public class ConfigTagsRelationMapperByPostgresqlTest {
    ...
    public void testFindConfigInfo4PageFetchRows() {
        ...
        Assert.assertEquals(
                "SELECT a.id,a.data_id,a.group_id,a.tenant_id,a.app_name,a.content FROM config_info  a LEFT JOIN config_tags_relation b ON a.id=b.id WHERE  a.tenant_id=?  AND a.data_id=?  AND a.group_id=?  AND a.app_name=?  AND a.content LIKE ?  AND b.tag_name IN (?, ?, ?, ?, ?)  OFFSET "+ startRow +" LIMIT " + pageSize
                         , mapperResult.getSql());
        List<Object> list = CollectionUtils.list(tenantId);
        list.add("dataID1");
        list.add("groupID1");
    ...
    public void tsetFindConfigInfoLike4PageFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                "SELECT a.id,a.data_id,a.group_id,a.tenant_id,a.app_name,a.content FROM config_info a LEFT JOIN config_tags_relation b ON a.id=b.id  WHERE  a.tenant_id LIKE ?  AND a.data_id LIKE ?  AND a.group_id LIKE ?  AND a.app_name = ?  AND a.content LIKE ?  AND b.tag_name IN (?, ?, ?, ?, ?)  OFFSET "+ startRow + " LIMIT " + pageSize);
        List<Object> list = CollectionUtils.list(tenantId);
        list.add("dataID1");
        list.add("groupID1");
        list.add("AppName1");
