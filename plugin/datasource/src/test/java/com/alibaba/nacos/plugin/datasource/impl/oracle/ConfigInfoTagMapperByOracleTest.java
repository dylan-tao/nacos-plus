...
public class ConfigInfoTagMapperByOracleTest {
    ...
    public void testUpdateConfigInfo4TagCas() {
        ...
        MapperResult mapperResult = configInfoTagMapperByOracle.updateConfigInfo4TagCas(context);
    ...
    public void testFindAllConfigInfoTagForDumpAllFetchRows() {
        ...
        Assert.assertEquals(mapperResult.getSql(),
                " SELECT t.id,data_id,group_id,tenant_id,tag_id,app_name,content,md5,gmt_modified  FROM ( SELECT * FROM ( SELECT id, ROWNUM as rnum FROM config_info_tag  ORDER BY id)  WHERE  rnum >= " + (startRow+1) + " and "+ pageSize +" >= rnum ) g, config_info_tag t  WHERE g.id = t.id  ");
