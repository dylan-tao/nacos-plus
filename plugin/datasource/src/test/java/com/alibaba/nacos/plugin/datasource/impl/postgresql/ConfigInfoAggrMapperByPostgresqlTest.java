...
public class ConfigInfoAggrMapperByPostgresqlTest {
    ...
    public void testBatchRemoveAggr() {
        ...
        MapperResult mapperResult = configInfoAggrMapperByPostgresql.batchRemoveAggr(context);
    ...
    public void testAggrConfigInfoCount() {
        ...
        context.putWhereParameter(FieldConstant.GROUP_ID, groupId);
        context.putWhereParameter(FieldConstant.TENANT_ID, tenantId);
        MapperResult mapperResult = configInfoAggrMapperByPostgresql.aggrConfigInfoCount(context);
        String sql = mapperResult.getSql();
        List<Object> paramList = mapperResult.getParamList();
    ...
    public void testFindConfigInfoAggrIsOrdered() {
        ...
        MapperResult mapperResult = configInfoAggrMapperByPostgresql.findConfigInfoAggrIsOrdered(context);
        String sql = mapperResult.getSql();
        List<Object> paramList = mapperResult.getParamList();
    ...
    public void testFindConfigInfoAggrByPageFetchRows() {
        ...
        MapperResult mapperResult = configInfoAggrMapperByPostgresql.findConfigInfoAggrByPageFetchRows(context);
        String sql = mapperResult.getSql();
        List<Object> paramList = mapperResult.getParamList();
        
        Assert.assertEquals(sql,
                "SELECT data_id,group_id,tenant_id,datum_id,app_name,content FROM config_info_aggr WHERE data_id= ? AND group_id= ? AND tenant_id= ? ORDER BY datum_id OFFSET 0 LIMIT 5");
        Assert.assertEquals(paramList, Arrays.asList(dataId, groupId, tenantId));
    }
    
    @Test
    public void testFindAllAggrGroupByDistinct() {
        MapperResult mapperResult = configInfoAggrMapperByPostgresql.findAllAggrGroupByDistinct(null);
