...
public class ConfigInfoAggrMapperByOracle extends AbstractMapper implements ConfigInfoAggrMapper {
    ...
    public MapperResult findConfigInfoAggrByPageFetchRows(MapperContext context) {
        ...
        String sql = "SELECT * FROM (SELECT data_id,group_id,tenant_id,datum_id,app_name,content, ROWNUM as rnum" + " FROM config_info_aggr WHERE data_id= ? AND " + "group_id= ? AND tenant_id= ? ORDER BY datum_id) WHERE  rnum >= " + (startRow + 1) + " and " + (pageSize + startRow) + " >= rnum";
